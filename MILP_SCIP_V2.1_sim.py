...
##                                                                                                                    ##) [f3 + f4 + f5 + f6 + ....fn-1+min_runtime] <= n_switching
    ...
    for k in range((min_runtime-1-2),(n-1)+min_runtime):    
        model.addCons((quicksum( HP_S_bin[k-j]  for j in range(min_runtime-2)) <= 1))                                                           #) [ -   f0   f1    f2 ] [f3]
...
hours = 24*7       # hours to solve each iteration
...
#filename ='HO_Summer_2_Weeks_EPEX_2018.06.28.xlsx'
pth = "//141.79.92.10/Common/INES/Technikum/KWKK_Labor/9_ConvCtrl_vs_OptCtrl/Simulation/Loadprofiles/"
filename = "HSG_Transition_1_Week_EPEX_20_07_2018.xlsx"
...
#%%       
    ...
    """------ Retrieve data from excel ----"""
    Pth_Load_Heat_File,Pth_Load_Cool_File,Time,Pel_Load,epex_price_buy,epex_price_sell,T_Amb = read_excel(pth + filename)      #Read excel file
...
#        mqtt_kwkk.mqtterror(solv_error)
    ...
    try:    
        ...
        model.setRealParam('limits/time',60*60*10)               # for limiting to n sec sec the solving time
...
#    WriteExcel("SIMULATION_OPT" + filename,Step_Pgrid_B,Step_Pgrid_S,Step_CHP_bin,Step_HP_bin,Step_COIL_bin,Step_T_HTES, Step_T_CTES, Step_CCM_bin, Step_AdCM_bin, T_Amb_f_15[:i+1], T_Amb_Forec_real_15[:i+1], Pth_Load_Heat[:i+1], Pth_Load_Cool[:i+1], EPEX_buy_list, EPEX_sell_list)
    WriteExcel("SIMULATION_OPT_" + filename,Pgrid_B_opt,Pgrid_S_opt,CHP_bin_opt,HP_bin_opt,COIL_bin_opt,T_HTES_opt[:-1], T_CTES_opt[:-1], CCM_bin_opt, AdCM_bin_opt, T_Amb_f_15[:n-1], T_Amb_Forec_real_15[:n-1], Pth_Load_Heat[:n-1], Pth_Load_Cool[:n-1], Par_C_El_B, Par_C_El_S)
